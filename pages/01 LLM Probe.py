...
csv_dataset_options = [os.path.basename(
    f).replace('.csv', '') for f in csv_files]
...
model_tracker = create_model_tracker(
    model_status, model_progress_bar, model_progress_text, model_detail, add_log)
dataset_tracker = create_dataset_tracker(
    dataset_status, dataset_progress_bar, dataset_progress_text, dataset_detail, add_log)
embedding_tracker = create_embedding_tracker(
    embedding_status, embedding_progress_bar, embedding_progress_text, embedding_detail, add_log)
training_tracker = create_training_tracker(
    training_status, training_progress_bar, training_progress_text, training_detail, add_log)

# Create a generic tracker for analysis (similar structure to other trackers)
analysis_tracker = create_training_tracker(
    analysis_status, analysis_progress_bar, analysis_progress_text, analysis_detail, add_log)
    ...
    try:
        ...
        if dataset_source in csv_dataset_options:
            ...
            try:
                ...
                # Set a name attribute for identification
                ...
                dataset_tracker.update(
                    1.0, f"Error loading {dataset_source} dataset", str(e))
        ...
        add_log(
            f"Saved parameters to {os.path.join(run_folder, 'parameters.json')}")
        ...
        analysis_tracker.update(
            0, "Starting analysis phase...", "Initializing visualizations and saving data")

        # 5. Plot and display results
        with main_tabs[0]:
            # Update progress for generating accuracy data
            analysis_tracker.update(
                0.1, "Generating accuracy data...", "Creating dataframes and saving results")
            ...
            save_json(results_to_save, os.path.join(
                run_folder, "results.json"))
            add_log(
                f"Saved results to {os.path.join(run_folder, 'results.json')}")

            # Update progress for saving representations
            analysis_tracker.update(
                0.2, "Saving model representations...", "Writing hidden states to disk")
            ...
            save_representations(train_hidden_states,
                                 train_representations_path)
            save_representations(test_hidden_states, test_representations_path)
            add_log(
                f"Saved train representations to {train_representations_path}")
            add_log(
                f"Saved test representations to {test_representations_path}")
            ...
            analysis_tracker.update(0.4, "Saving detailed per-layer visualizations...",
                                    "Generating and saving visualizations for each layer")
            ...
            for layer in range(num_layers):
                ...
                if diff_activations is not None:
                    ...
                    ax_diff.set_title(
                        f"Mean Activation Difference (True - False) - Layer {layer}")
                    ...
                    plt.tight_layout()
                    save_graph(fig_diff, os.path.join(
                    ...
                    if layer < len(results['probes']):
                        ...
                        fig_weights = plot_probe_weights(
                            probe, layer, run_folder)
                        save_graph(fig_weights, os.path.join(
                        ...
                        save_graph(fig_proj, os.path.join(
                        ...
                        fig_conf = plot_confusion_matrix(
                            metrics, layer, run_folder)
                        save_graph(fig_conf, os.path.join(
                        ...
                        if hasattr(probe, 'linear') and hasattr(probe.linear, 'weight'):
                            ...
                            save_graph(fig_align, os.path.join(
                                layer_save_dir, "neuron_alignment.png"))
                            plt.close(fig_align)

            add_log("Per-layer visualizations saved successfully")

            # --- Calculate Alignment Strengths for all layers ---
            analysis_tracker.update(0.7, "Calculating alignment strengths...",
                                    "Analyzing neuron alignment between probe weights and activation differences")
            ...
            with accuracy_tab_container:  # Display in the first sub-tab of Probe Analysis
                if use_control_tasks and results['selectivities']:
                    ...
                    save_graph(fig_sel, os.path.join(
                    ...
                    save_graph(fig_acc, os.path.join(
                        run_folder, "accuracy_plot.png"))
                    with st.expander("What does this chart show?", expanded=False):
                        st.markdown("""
                        This chart visualizes the performance of the linear truth probes across different layers of the model.
            ...
            with alignment_tab_container:
                alignment_strength_plot.info(
                    "Generating Alignment Strength visualization...")
                if alignment_strengths:
                    ...
                    save_graph(fig_align_strength, os.path.join(
                    ...
                    **Interpretation:**
                    ...
                    alignment_strength_plot.info(
                        "Alignment strength data could not be computed.")
                ...
                for i, layer_tab in enumerate(layer_tabs):
                    with layer_tab:
                        ...
                        if results and 'probes' in results and selected_layer < len(results['probes']):
                            ...
                            fig_probe_weights = plot_probe_weights(
                                probe, selected_layer, run_folder)
                        ...
                        if 'diff_activations' in locals() and diff_activations is not None and \
                           results and 'probes' in results and selected_layer < len(results['probes']):
                            ...
                            top_k_data = get_top_k_neurons(
                                diff_activations, current_probe_weights, k=10)
                        ...
                        with col2:
                            ...
                            fig_confusion = plot_confusion_matrix(
                                metrics, selected_layer, run_folder)
                            ...
                            with torch.no_grad():
                                ...
                                correct_indices = (test_preds == test_labels).nonzero(
                                    as_tuple=True)[0]

                                # Get incorrect examples
                                incorrect_indices = (
                                    test_preds != test_labels).nonzero(as_tuple=True)[0]
                                ...
                                if len(correct_indices) > 0:
                                    for i in range(min(3, len(correct_indices))):
                                        ...
                                        pred = "True" if test_preds[idx].item(
                                        ) == 1 else "False"
                                        conf = test_probs[idx].item(
                                        ) if pred == "True" else 1 - test_probs[idx].item()
            ...
            analysis_tracker.update(
                0.8, "Analysis complete!", "All visualizations and data saved successfully")
